...
/* velikost mapy */
#define ROWS 15 // řádky
#define COLS 45 // sloupce

/* velikost okna */
#define CONSOLE_WIDTH 400 // šířka
...
int main() {
	...
	/* boční otvory na přechod */
	placeBlockSymetricY(AIR, 0, ROWS>>1);
	
	/* vygeneruje vnitřní grid */
	...
	while(1) {
		...
		/* v případě když je hráč mimo mapu, přesune ho to na druhou stranu */
...
void drawMap() {
	for (int y = 0; y < ROWS; y++) {
		for (int x = 0; x < COLS; x++) {
			...
			/* jestli je na pozici nepřátel, vykresli */
			bool founded = false;
			for (int e = 0; e < 10; e++) {
				if (!enemies[e].alive) continue;
			...
			/* jestli tam je hráč, vykresli */
			if (x == playerX && y == playerY) {
				cout << (char) 2;
				continue;
			}
			
			/* ostatní pasivní objekty na mapě */
			switch (map[y][x]) {
				case BLOCK:
